...
export function VehicleDetailPage() {
  ...
  const rows = [["Tipo", vehicle.type === "truck" ? "Camion" : "Remolque"], ["Marca", vehicle.brand || "-"], ["Modelo", vehicle.model || "-"], ["Estado", vehicleStatusLabels[vehicle.status] || vehicle.status], ["Chofer asignado", vehicle.currentDriver?.fullName || "-"]];
  return <><div className="d-flex justify-content-between align-items-center flex-wrap gap-2 mb-4"><h1 className="h3 mb-0">Detalle de vehiculo</h1><button className="btn btn-outline-primary" onClick={() => navigate("/vehiculos")}>Volver a vehiculos</button></div><div className="card shadow-sm"><div className="card-header"><strong>{vehicle.licensePlate}</strong></div><div className="card-body"><div className="row g-3">{rows.map(([label, value]) => <div className="col-md-4" key={label}><div className="small text-muted">{label}</div><div>{value}</div></div>)}</div></div></div></>;
}
