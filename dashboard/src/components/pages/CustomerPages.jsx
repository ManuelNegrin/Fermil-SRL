...
export function CustomersPage() {
  ...
  const submit = async (event) => {
    ...
    try {
      if (editing) { await apiFetch(`/api/clientes/${editing.id}`, { method: "PUT", body: form }); toast.success("Cliente actualizado."); }
  ...
  const deactivate = async (customer) => {
    if (!window.confirm(`Dar de baja a ${customer.name}?`)) return;
